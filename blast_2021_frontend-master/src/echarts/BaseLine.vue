...
export default {
  ...
  computed: {
    lastXAxis() {
        if (this.data.x && this.data.x.length > 0) {
            return this.data.x[this.data.x.length - 1];
        } else {
            return null;
        }
    },
    ...
    series_data: function () {
      return this.data.y.map((cur, index) => {
        return {
          ...
          markPoint: this.granularity==='ten_minutes'?
          {
                    data: [
                        { name: "缺失", value: null, xAxis: this.lastXAxis, yAxis: 50, itemStyle: { color: '#FF0000' } }
                    ],
                    symbolSize: 20 // 调整标注点的大小为 10
                }
                 :null,

          markArea: this.markArea ? this.markArea_config : null,
          markLine: this.markLine//60那条线
              ? {
                // silent: true,
                symbol: ["none", "none"],
                lineStyle: {
