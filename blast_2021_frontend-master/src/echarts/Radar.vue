...
export default {
  ...
  methods: {
    ...
    processData(name, raw_data) {
      let indicator = [];
      let value = []
      let key_list = ["下料稳定性", "压量关系稳定性", "炉缸工况稳定性", "操作炉型稳定性", "煤气流分布稳定性"]  //确保都是按照这个顺序的
      key_list.forEach((key) => {
        // console.log(key)
        // console.log(raw_data)
        // console.log(value)
        indicator.push({text: key.replace("稳定性", ""), max: 1*100});
        value.push(raw_data[key] != null ? raw_data[key]['subIndexValue'].toFixed(2)*100 : '暂无')
      })
  ...
  computed: {
    ...
    series: function () {
      let res = [];
      let blast_data = [this.ten_minutes_data, this.hour_data, this.class_data, this.day_data, this.week_data];
    ...
    radar_option: function () {
      return {
        ...
        legend: {
          ...
          selected:{"十分钟级":false, "小时级":true, "班次级":false, "天均":false, "周均":false},
