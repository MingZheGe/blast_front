<template>
  <div>
    <p class="light_text" style="text-align:center;font-size:30px">总体稳定性指数值: <span :style="{color: value2color(index_data)}">{{this.index_formatter(index_data).toFixed(0)}}</span></p>
    <BlastImg :text="this.sub_index_data" :text_color="color_data" :container_width="container_width"/>
  </div>
</template>
...
export default {
  ...
  computed:{
    ...
    sub_index_data(){
      ...
      ].map(this.index_formatter).map(value=>Math.round(value).toFixed(0));
    }
  },
  data() {
    return {
