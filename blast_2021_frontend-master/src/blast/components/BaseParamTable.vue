<template>
  <div>
    <p class="title " style="font-size:110%;color:lightgray">{{ caption }}</p>
    <div style="background: rgba(25, 25, 112, 0.7);font-size:80%;color: orange">
      <b-table :busy="isBusy"
               dark
               caption-top
               striped
      ...
      >
        ...
        <template #cell(value_1)="data">
          <div @click="table_click(data,1)"> {{ dataFormatter(data.item.value_1, data.item.name_1) }}</div>
        </template>

        <template #cell(value_2)="data">
          <div @click="table_click(data,2)"> {{ dataFormatter(data.item.value_2, data.item.name_2) }}</div>
        </template>
        ...
        <template #table-colgroup="scope">
          <col
              ...
              :key="field.key"
              :style="{ width: field.key.includes('value')  ? '150px' : '150px' }"
          />
        </template>
        <template #table-busy>
...
export default {
  ...
  methods: {
    dataFormatter(value, name) {
      if(name===""){ //最后一行的空位置的value是字符类型，不能用toFIxed
        return "";
      }
      if (name.indexOf("温度") !== -1 || name.indexOf("热负荷") !== -1||name.indexOf("鼓风动能") !== -1) {
    ...
    table_click(data, col_index) {
      ...
      if (col_index === 1) {
        ...
        this.$emit("table_click", {id: data.item.id_1, name: data.item.name_1})
      } else if (col_index === 2) {
        console.log(data.item.name_2)
        console.log(data.item.id_2)
        this.$emit("table_click", {id: data.item.id_2, name: data.item.name_2})
      }
